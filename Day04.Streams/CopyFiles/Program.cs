    ...
    public class Program
    {
        ...
        public static void BlockCopy(string source, string destin)
        {
            ...
            using (var destinStream = new FileStream(destin, FileMode.Create, FileAccess.Write))
            {
                ...
                do
                {
                    bytesRead = sourceStream.Read(buffer, 0, 1024); // TODO: read in buffer
        ...
        public static void MemoryBufferCopy(string source, string destin)
        {
            ...
            using (
                var textReader = (TextReader) new StreamReader(new FileStream(source, FileMode.Open, FileAccess.Read)))
                // TODO: use StreamReader here
            {
            ...
            using (var destinStream = new FileStream("NewTextFile.txt", FileMode.Create, FileAccess.Write))
                // TODO: Use file stream.
            {
                ...
                do
                {
                    bytesRead = stringReader.ReadBlock(buffer, 0, blockSize);
                        // TODO: Read block from stringReader to buffer.
                    ...
                    //Console.WriteLine((int)memoryStream.Length);
                } while (bytesRead == blockSize);
                // TODO: write memoryStream.GetBuffer() content to destination stream.
                //streamWriter.Flush();
                destinStream.Write(memoryStream.GetBuffer(), 0, (int)memoryStream.Length);
            }
        }
        ...
        public static void WebClient()
        {
            ...
            using (var stream = webClient.OpenRead("http://google.com"))
            {
                ...
                Console.WriteLine("WebClient(): CanRead = {0}", stream.CanRead.ToString());
                    // TODO: print if it is possible to read from the stream
                Console.WriteLine("WebClient(): CanWrite = {0}", stream.CanWrite.ToString());
                    // TODO: print if it is possible to write to the stream
                Console.WriteLine("WebClient(): CanSeek = {0}", stream.CanSeek.ToString());
                    // TODO: print if it is possible to seek through the stream


                // TODO: Save steam content to "google_request.txt" file.
                using (var streamReader = new StreamReader(stream))
                using (var destinStream = new FileStream("google_request.txt", FileMode.Create, FileAccess.Write))
                using (var streamWriter = new StreamWriter(destinStream))
                {
